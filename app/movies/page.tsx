...
export default function MoviesPage() {
    ...
    const load = async () => {
        const all = await repo.listEntriesByTypeAndStatus('movie');
        setItems(all);
        setLoading(false);
    };
            ...
            <div className="mx-auto max-w-[1200px]">
                <header className="mb-8 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <h1 className="text-3xl font-bold font-display">Movies</h1>
                    </div>
                ...
                <div className="space-y-12">
                    ...
                    {items.length === 0 && (
                        <div className="text-center text-muted-foreground py-20 bg-muted/20 rounded-xl">
                            No movies tracked yet. <br />
                            <Button variant="link" onClick={() => setSearchOpen(true)}>Add your first movie</Button>
                        </div>
                    )}
                </div>
            </div>
